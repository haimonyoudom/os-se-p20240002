...
| Initial stock | 100 |
| Concurrent buyers (swarm) | 60 |
| Worker threads | 4 |
| Backups to retain | 4 |
| Cron test interval | every 1 minute |
| One-shot timed job fires at | 14:35 today |
| Your student ID | p20240002 |

**Narrative:** You are the on-call systems engineer at **`TechCorp`**. A launch of
...
exact tree; only `p20240002` changes.

```text
~/os-se-p20240002/
├── os-lab-p20240002/        ← your existing labs (already committed)
├── os-class-activities-p20240002/   ← your existing activities
...
> `cd ~/os-se-p20240002/final-exam && unzip ~/exam-paper/final-exam-templates.zip`
...
1. **A1 — Concurrency.** Write `thread_demo.c` spawning exactly `4`
   ...
   - 📸 `a1_thread_run.png` — output showing all `4` workers + summary.
...
1. **D1 — The engine.** Write `buy_reactor_core`: takes a buyer name and a
   ...
   `100`, and on success decrements stock and appends a sales-log line
   that includes **your real student id**.
2. **D2 — The race.** Write `swarm`: launches `60` concurrent purchases in
   the background and waits. Reset stock to `100`, run several times, and
...
stock than the correct final value (with `100` stock and `60`
concurrent buyers)?
...
1. **E1 — `backup_project`.** Create a small sample project (a few files in a couple
   ...
   archive on each run and keep only the newest `4` archives, pruning the
   rest. Run it enough times that pruning happens.
   - 📸 `e1_backup_retention.png` — exactly `4` archives left + the log.
